...
function UserProfile() {
  ...
  const activityCount = filteredItineraries.reduce(
    (totalActivities, itinerary) => {
      return (
        totalActivities +
  ...
  if (filteredItineraries.length === 0)
    return (
      <main>
        <div className="text-in-empty-scree">
          This user has not created any itineraries yet. Please check back
          later.
        </div>
      </main>
    );
      ...
      <div className="traveler-personal">
        <div className="traveler-personal-profile">
          <img
            ...
            src={filteredItineraries[0].traveler.profile_url}
            alt={travelerId}
          />
        ...
        <div className="landing-personal-container">
          <div className="travel-stats">
            <div>
              {filteredItineraries[0].traveler.first_name} has been on{" "}
              <span className="highlight">{itineraryCount}</span> trips
            </div>
            <div>
              has traveled for a total of{" "}
              <span className="highlight">{scheduleCount}</span> days
            </div>
            <div>
              and been to <span className="highlight">{activityCount}</span>{" "}
              places
            </div>
          </div>
        </div>
